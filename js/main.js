...
$(document).ready(function () {
  ...
  $("#btnJSON").click(function (e) {
    if (global_json != 0) {
      $("#dataJSON").html(
        syntaxHighlight(JSON.stringify(JSON.parse(global_json), null, 4))
      );
...
$(document).ready(function () {
  function generateSignature() {
    ...
    let consid = localStorage.getItem("consid");
    let secret = localStorage.getItem("secret");

    // Timestamp (detik sejak epoch UTC)
    let tstamp = Math.floor(Date.now() / 1000).toString();

    // Bikin signature (HMAC-SHA256)
    let rawSignature = CryptoJS.HmacSHA256(consid + "&" + tstamp, secret);

    // Convert ke Base64
    let encodeSignature = CryptoJS.enc.Base64.stringify(rawSignature);

    // Set ke input
    $("#x-timestamp").val(tstamp);
    $("#x-signature").val(encodeSignature);

    // Debug log
    // console.log("tstamp:", tstamp);
    // console.log("signature:", encodeSignature);
  }

  // Pertama kali langsung generate
  generateSignature();

  // Auto regenerate tiap 10 detik (10000 ms)
  setInterval(generateSignature, 10000);
});
